...
class Scheduler:
    ...
    def _start_all_routers(self):
        with self.lock:
            for router in config_manager.get_routers():
                self._start_router_thread(router)

    def _start_router_thread(self, router: RosRouter):
    ...
    def _stop_router_thread(self, router_id: str):
    ...
    def on_router_added(self, router: RosRouter):
        with self.lock:
            if not self.stop_event.is_set():
                self._start_router_thread(router)

    def on_router_updated(self, router: RosRouter):
        with self.lock:
            if router.id in self.threads:
    ...
    def on_router_deleted(self, router_id: str):
        with self.lock:
